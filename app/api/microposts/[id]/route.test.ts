...
describe("DELETE /api/microposts/[id]", () => {
  ...
  it("自分のMicropostなら削除して204を返す", async () => {
    ...
    // 画像なしの投稿なので、R2の削除処理は呼ばれない。
    expect(deleteImage).not.toHaveBeenCalled();

    expect(response.status).toBe(204);
  });

  it("画像付きMicropostならDBとR2から削除して204を返す", async () => {
    vi.mocked(currentUser).mockResolvedValue({
      id: "user-id",
      name: "Hamru",
      email: "hamru@example.com",
    ...
    vi.mocked(prisma.micropost.findUnique).mockResolvedValue({
      ...
      content: "画像付き投稿",
      userId: "user-id",
      imageKey: "test/microposts/test-image.jpg",
      createdAt: new Date(),
      updatedAt: new Date(),
    });
    ...
    // imageKeyがあるので、R2の画像削除処理も1回呼ばれる。
    expect(deleteImage).toHaveBeenCalledOnce();

    // DBから取得したimageKeyが、そのままR2の削除処理へ渡される。
    expect(deleteImage).toHaveBeenCalledWith("test/microposts/test-image.jpg");
